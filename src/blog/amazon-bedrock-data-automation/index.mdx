...
Focus recently completed a short engagement helping a client configure and evaluate the suitability of [Amazon Bedrock Data Automation](https://aws.amazon.com/bedrock/bda/) (BDA) as part of a benefits eligibility processing system.

## A Brief Introduction to BDA

Among other capabilities, BDA provides GenAI-powered turnkey document classification and data extraction. It is configured by creating "blueprints" of the different types of documents which it will process and aggregating those into "projects" to which documents can be submitted via its API. Amazon offers a set of pre-defined ("catalog") blueprints for common document types. However, only a couple of the client's document types matched the catalog, and the client's data minimization requirements around Personally Identifiable Information (PII) meant we needed tighter control over what got extracted regardless. So we opted to define custom blueprints for all of the client's documents.
...
Notice the `instruction` and `description` properties in the `schema` above; they are the tunable levers affecting document classification and field data extraction. The `instruction` field is capped at 600 characters depending on blueprint type. A BDA project can contain up to 40 DOCUMENT type blueprints and 1 IMAGE type blueprint.
...
Documents which were native PDFs produced excellent accuracy and confidence results, but images produced varying results which were highly dependent on input quality. This second class of documents is the one for which we spent the most time investigating failures and tweaking blueprint schema parameters to attempt to improve results. Here are some of the specific problems we encountered:
...
* **Document name can lead to misclassification**. A self-attestation letter which mentioned the string "1040" was classified as such, but thankfully it had a low confidence score.

## Conclusion

Amazon BDA proved to be a capable system for automating document classification and data extraction, with strong performance out of the box on native PDFs and results on image-based documents that required deliberate tuning to reach production-level accuracy. The API is well designed and clearly documented, and the relatively small set of tunable parameters kept the system approachable, even though getting the best results required real investigation into how those parameters interact.
